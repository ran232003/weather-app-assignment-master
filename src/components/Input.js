...
const Input = (props)=>{
    ...
    const dispatch =  useDispatch()
        const changeInput = async(event)=>{
            ...
            const response = await fetch(`https://dataservice.accuweather.com/locations/v1/cities/autocomplete?apikey=${apiKey}&q=${val}&language=en-us`)
        ...
        const getInput = async(event,value)=>{
            ...
            const res2 = await fetch(`https://dataservice.accuweather.com/currentconditions/v1/${id}?apikey=${apiKey}&language=en-us&details=true`)
    const data2 =  await res2.json();
     dispatch(weatherActions.setCurrentWeather(data2[0]));
     const res = await fetch(`https://dataservice.accuweather.com/forecasts/v1/daily/5day/${id}?apikey=${apiKey}&language=en-us&details=false&metric=true`)
